...
def validate_token(token):
    try:
        response = requests.get(
            "http://localhost:8080/api/auth/validate",  # Spring Boot endpoint
            headers={"Authorization": f"Bearer {token}"}
        )
        return response.status_code == 200
    except Exception as e:
        st.error("Token validation error: " + str(e))
        return False
...
# --- Main Function with Token Gate ---
def main():
    st.markdown('<div class="title">🔐 Secure MediBot</div>', unsafe_allow_html=True)
    ...
    if not st.session_state.authenticated:
        ...
        if token:
            ...
            st.info("Enter a valid JWT token to access the chatbot.")
    else:
        run_chatbot()
